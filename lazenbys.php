...
if($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['addToCart'])){
	...
	for($i = 0; $i < $numOrder; $i++){
		...
		$orderDetail = $conn->prepare("INSERT INTO cart (cafe, itemID, quantity, collecttime, customer, remark, orderdate, price) VALUES ('lazenbys', ?, ?, ?, ?, ?, ?, ?)");
...
//Get opening hours
$getOpen = "SELECT * FROM open WHERE ID='1'";
...
//GET menu items
$getItem = "SELECT * FROM lazenbys WHERE ".$weekday[($nextDay)]."=1 AND manselect=1";
...
<!--Picture of the page-->
<div id='carousel'>
	<img src='./img/coffee.jpg' alt='Lazenbys'>
</div>

<!--Banner in middle-->
<div class='heading' id='lazenbysHeading'>Lazenbys</div>
		...
		<div class='col-lg-7 col-12'>
			...
			<?php
				...
				if(preg_match('/1$/', $_SESSION['permission']) || $_SESSION['permission']==400){
					echo "<div class='col-12'>
					<a href='./lazenbys_order.php'><button type='button' class='btn-success mb-1'>&#9778; View Orders</button></a>
					</div>";
				};


				//Button to manager page for manager and director
				if($_SESSION['permission']==301 || $_SESSION['permission']==400){
					echo "<div class='col-12'>
					<a href='./lazenbys_manager.php'><button type='button' class='btn-warning mb-1'>&#8499; Manager Page</button></a>
					</div>";
				};
				...
				if($_SESSION['permission']==400){
					echo "<div class='col-12'>
					<a href='./lazenbys_master.php'><button type='button' class='btn-alert mb-1'>&#10000; Master Page</button></a>
					</div>";
				} 
			?>
		</div>
