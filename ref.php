...
if($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['addToCart'])){
	...
	for($i = 0; $i < $numOrder; $i++){
		...
		$orderDetail = $conn->prepare("INSERT INTO cart (cafe, itemID, quantity, collecttime, customer, remark, orderdate, price) VALUES ('ref', ?, ?, ?, ?, ?, ?, ?)");
...
//Get opening hours
$getOpen = "SELECT * FROM open WHERE ID='2'";
...
//GET menu items
$getItem = "SELECT * FROM ref WHERE ".$weekday[($nextDay)]."=1 AND manselect=1";
...
<!--Picture of the page-->
<div id='carousel'>
	<img src='./img/sandwich.jpg' alt='The Ref'>
</div>

<!--Banner in middle-->
<div class='heading' id='theRefHeading'>The Ref</div>
		...
		<div class='col-lg-7 col-12'>
			...
			<?php
				...
				if(preg_match('/2$/', $_SESSION['permission']) || $_SESSION['permission']==400){
					echo "<div class='col-12'>
					<a href='./ref_order.php'><button type='button' class='btn-success mb-1'>&#9778; View Orders</button></a>
					</div>";
				};


				//Button to manager page for manager and director
				if($_SESSION['permission']==302 || $_SESSION['permission']==400){
					echo "<div class='col-12'>
					<a href='./ref_manager.php'><button type='button' class='btn-warning mb-1'>&#8499; Manager Page</button></a>
					</div>";
				};
				...
				if($_SESSION['permission']==400){
					echo "<div class='col-12'>
					<a href='./ref_master.php'><button type='button' class='btn-alert mb-1'>&#10000; Master Page</button></a>
					</div>";
				} 
			?>
		</div>
