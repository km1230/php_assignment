...
if($_SESSION['permission']!=400 && $_SESSION['permission']!=302){
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if(isset($_POST['updateHour'])){
		...
		$updatetime = "UPDATE open SET openhour='".$oh."', closehour='".$ch."' WHERE ID='2'";
	...
	if(isset($_POST['updateMenu'])){
		...
		if($manSelect[1] == 0){
			...
			for($i = 0;$i < $numItem; $i++){
				$updateMenu = "UPDATE ref SET manselect='".$_POST['manselect'][$i]."' WHERE ID='".$_POST['itemID'][$i]."'";
...
//get item list
$getItem = "SELECT * FROM ref WHERE $weekday[$nextDay]=1";
$getItemResult = $conn->query($getItem);

//get opening hour
$getTime = "SELECT * FROM open WHERE ID='2'";
...
<!--Banner in middle-->
<div class='heading'>The Ref Manager Page</div>
...
<div class='mb-3'>
	<ul class='breadcrumb'>
		...
		<li>
			<a href='./ref_manager.php'>::The Ref Manager Page::</a>
		</li>
	</ul>
</div>
