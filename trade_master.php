...
if($_SERVER['REQUEST_METHOD']==='POST'){
	...
	if(isset($_POST['newItem'])){
		...
		if(!$duplicated){
			...
			$addNewItem = $conn->prepare("INSERT INTO trade (item) VALUES (?)");
		...
		for($i = 0; $i < $numItem; $i++){
			...
			$editItem = $conn->prepare("UPDATE trade SET item=?, price=?, mon=?, tue=?, wed=?, thu=?, fri=? WHERE ID=?");
		...
		if($toBeRemove[1] == $numItem){
			...
			for($i = 0;$i < $numItem; $i++){
				if($_POST['removeItem'][$i] == 1){
					$removeItem = "DELETE FROM trade WHERE ID='".$_POST['itemID'][$i]."'";
...
<!--Banner in middle-->
<div class='heading'>Trade Table Master Food &amp; Beverage List</div>
...
<div class='mb-3'>
	<ul class='breadcrumb'>
		...
		<li>
			<a href='./trade_master.php'>::Trade Table Master Food &amp; Beverage List::</a>
		</li>
	</ul>
</div>
